...
dx = 0.01
dy = 0.01
N = 80##nombre de formes aléatoires
dir_vtu = 'D:/DS0/vtus/'
...
for index in 80+np.arange(N):
    ...
    with open(dir_matrices + 'u/shape_{}.pickle'.format(index), 'wb') as handle1:
        pickle.dump(uu, handle1, protocol=pickle.HIGHEST_PROTOCOL)
    with open(dir_matrices + 'v/shape_{}.pickle'.format(index), 'wb') as handle2:
        pickle.dump(vv, handle2, protocol=pickle.HIGHEST_PROTOCOL)
    with open(dir_matrices + 'p/shape_{}.pickle'.format(index), 'wb') as handle3:
        pickle.dump(pp, handle3, protocol=pickle.HIGHEST_PROTOCOL)
    with open(dir_matrices + 'velocities/shape_{}.pickle'.format(index), 'wb') as handle4:
        pickle.dump(velocity, handle4, protocol=pickle.HIGHEST_PROTOCOL)
    with open(dir_matrices + 'shapes/shape_{}.pickle'.format(index), 'wb') as handle5:
