...
def load_drag_lift_dataset(my_dir, n_outputs):
    ...
    bar  = progress.bar.Bar('Loading drag-lift from '+my_dir, max=n_sols)
...
def plot_accuracy_and_loss(train_model):
    ...
    np.savetxt('accuracy_and_loss',np.transpose([epochs,train_acc,valid_acc]))

    f, ax = plt.subplots(1,2, figsize=(14,6))
    ax[0].plot(epochs, train_acc, 'g', label='Training accuracy')
    ax[0].plot(epochs, valid_acc, 'r', label='Validation accuracy')
    ax[0].set_title('Training and validation accuracy')
    ax[0].legend()
    ax[0].set_xlabel('Epochs')
    ax[0].set_ylabel('Accuracy')
    ax[1].plot(epochs, train_loss, 'g', label='Training loss')
    ax[1].plot(epochs, valid_loss, 'r', label='Validation loss')
...
def load_model(model_json, model_h5):
    if ((not os.path.isfile(model_json)) or (not os.path.isfile(model_h5))):
